...
class BoolVector implements VectorInterface
{
    ...
    public function offsetGet($index)
    {
        ...
        $byteIndex = $index >> 3;
        $value = $this->primarySource[$byteIndex];
        $value = "\0" !== ($value & $mask[$index & 7]);
    ...
    public function offsetSet($index, $value)
    {
        ...
        if ($value) {
            $byteIndex = $index >> 3;
            $unassignedCount = $byteIndex - \strlen($this->primarySource) / 1;
            if ($unassignedCount < 0) {
                // Case 1. Overwrite an existing item.
                $this->primarySource[$byteIndex] = ($this->primarySource[$byteIndex] ?? "\0") | $originalMask[$index & 7];
            } elseif (0 === $unassignedCount) {
                // Case 2. Append an element right after the last one.
                $this->primarySource .= ($this->primarySource[$byteIndex] ?? "\0") | $originalMask[$index & 7];
            } else {
                // Case 3. Append to a gap after the last element. Fill the gap with default values.
                $this->primarySource .= \str_repeat("\0", (int) $unassignedCount).(($this->primarySource[$byteIndex] ?? "\0") | $originalMask[$index & 7]);
            }
        } else {
            $byteIndex = $index >> 3;
            $unassignedCount = $byteIndex - \strlen($this->primarySource) / 1;
            if ($unassignedCount < 0) {
                // Case 1. Overwrite an existing item.
                $this->primarySource[$byteIndex] = ($this->primarySource[$byteIndex] ?? "\0") & $invertedMask[$index & 7];
            } elseif (0 === $unassignedCount) {
                // Case 2. Append an element right after the last one.
                $this->primarySource .= ($this->primarySource[$byteIndex] ?? "\0") & $invertedMask[$index & 7];
            } else {
                // Case 3. Append to a gap after the last element. Fill the gap with default values.
                $this->primarySource .= \str_repeat("\0", (int) $unassignedCount).(($this->primarySource[$byteIndex] ?? "\0") & $invertedMask[$index & 7]);
            }
        }
    ...
    public function offsetUnset($index)
    {
        if (\is_int($index) && $index >= 0 && $index < $this->elementCount) {
            ...
            if ($this->elementCount > $index) {
                $carry = "\0";
                $byteCount = \strlen($this->primarySource) / 1;
                for ($i = $byteCount - 1, $byteIndex = $index >> 3; $i > $byteIndex; --$i) {
                    $this->primarySource[$i] = $shiftOneRight[$byte = $this->primarySource[$i]] | $carry;
                    $carry = "\1" === ($byte & "\1") ? "\200" : "\0";
                }
                // https://graphics.stanford.edu/~seander/bithacks.html#MaskedMerge
                $byte = $this->primarySource[$i];
                $this->primarySource[$i] = $byte ^ ($byte ^ ($shiftOneRight[$byte] | $carry)) & $mask[$index & 7];
            }
            if (0 === ($this->elementCount & 7)) {
                $this->primarySource = \substr($this->primarySource, 0, -1);
            }
        }
    }
    ...
    public function jsonSerialize(): array
    {
        ...
        for ($getIteratorIndex = 0; $getIteratorIndex < $elementCount; ++$getIteratorIndex) {
            ...
            $element = $this->primarySource[$byteIndex];
            $element = "\0" !== ($element & $mask[$getIteratorIndex & 7]);
            $result[] = $element;
        }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\BoolVector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        foreach ($elements as $element) {
            if (true === $element) {
                ...
                throw new \TypeError(self::EXCEPTION_PREFIX.\sprintf('Value must be of type %s%s, %s given', 'bool', '', \gettype($element)));
            }
            ++$howManyBitsToInsert;
