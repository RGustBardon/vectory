...
class Int16Vector implements VectorInterface
{
    ...
    public function offsetGet($index)
    {
        ...
        return \unpack('s', $packedInteger)[1];
    }
    ...
    public function offsetSet($index, $value)
    {
        ...
        if ($unassignedCount < 0) {
            ...
            $this->primarySource .= \str_repeat("\0\0", (int) $unassignedCount).$packedInteger;
        }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\Int16Vector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        $defaultValue = 0;
        $defaultValue = \pack('s', $defaultValue);
        $substringToInsert = '';
        foreach ($elements as $element) {
            ...
            if ($element < -32768 || $element > 32767) {
                throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '.-32768 .' <= x <= '. 32767);
            }
            $packedInteger = \pack('s', $element);
    ...
    private function deleteBytes(bool $primarySource, int $firstIndex, int $howMany, int $elementCount): void
    {
        if ($howMany >= $elementCount - $firstIndex) {
            if ($primarySource) {
                $this->primarySource = \substr($this->primarySource, 0, $firstIndex * 2);
                $this->elementCount = $firstIndex;
            }
        } else {
            if ($primarySource) {
                $this->primarySource = \substr_replace($this->primarySource, '', $firstIndex * 2, $howMany * 2);
    ...
    private function insertBytes(string $substringToInsert, int $firstIndex): void
    {
        ...
        if (-1 === $firstIndex || $firstIndex > $this->elementCount - 1) {
            ...
            $insertedElementCount = (int) (\strlen($substringToInsert) / 2);
