...
class Int64Vector implements VectorInterface
{
    ...
    public function offsetSet($index, $value)
    {
        ...
        if ($unassignedCount < 0) {
            ...
            $this->primarySource .= \str_repeat("\0\0\0\0\0\0\0\0", (int) $unassignedCount).$packedInteger;
        }
    ...
    public function jsonSerialize(): array
    {
        ...
        $elementCount = $this->elementCount;
        for ($getIteratorIndex = 0; $getIteratorIndex < $elementCount; ++$getIteratorIndex) {
            $packedInteger = \substr($this->primarySource, $getIteratorIndex * 8, 8);
            $element = \unpack('q', $packedInteger)[1];
            $result[] = $element;
        }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\Int64Vector', '\\')]]);
        ...
        try {
            $expectedLength = $this->elementCount * 8;
    ...
    private function deleteBytes(bool $primarySource, int $firstIndex, int $howMany, int $elementCount): void
    {
        if ($howMany >= $elementCount - $firstIndex) {
            if ($primarySource) {
                $this->primarySource = \substr($this->primarySource, 0, $firstIndex * 8);
                $this->elementCount = $firstIndex;
            }
        } else {
            if ($primarySource) {
                $this->primarySource = \substr_replace($this->primarySource, '', $firstIndex * 8, $howMany * 8);
    ...
    private function insertBytes(string $substringToInsert, int $firstIndex): void
    {
        ...
        if (-1 === $firstIndex || $firstIndex > $this->elementCount - 1) {
            ...
            $insertedElementCount = (int) (\strlen($substringToInsert) / 8);
