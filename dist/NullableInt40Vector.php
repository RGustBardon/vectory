...
class NullableInt40Vector implements VectorInterface
{
    ...
    public function getIterator(): \Traversable
    {
        ...
        foreach (\unpack('P*', \chunk_split($primarySource, 5, "\0\0\0")."\0\0\0") as $element) {
            ...
            (yield $bitIndex => "\0" === ($nullabilityByte & $mask[$bitIndex & 7]) ? $element > 549755813887 ? 549755813887 - $element : $element : null);
            ++$bitIndex;
        }
    }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\NullableInt40Vector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        foreach ($elements as $element) {
            if (null === $element) {
                ...
                if ($element < -549755813888 || $element > 549755813887) {
                    throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '.-549755813888 .' <= x <= '. 549755813887);
                }
                $packedInteger = \pack('PXXX', $element >= 0 ? $element : -$element + 549755813887);
