...
class NullableInt56Vector implements VectorInterface
{
    ...
    public function getIterator(): \Traversable
    {
        ...
        foreach (\unpack('P*', \chunk_split($primarySource, 7, "\0")."\0") as $element) {
            if (0 === ($bitIndex & 7)) {
                $nullabilityByte = $nullabilitySource[$bitIndex >> 3];
            }
            (yield $bitIndex => "\0" === ($nullabilityByte & $mask[$bitIndex & 7]) ? $element > 36028797018963967 ? 36028797018963967 - $element : $element : null);
            ++$bitIndex;
        }
    }
    ...
    public function jsonSerialize(): array
    {
        ...
        foreach (\unpack('P*', \chunk_split($primarySource, 7, "\0")."\0") as $element) {
            ...
            $jsonData[] = "\0" === ($nullabilityByte & $mask[$bitIndex & 7]) ? $element > 36028797018963967 ? 36028797018963967 - $element : $element : null;
            ++$bitIndex;
        }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\NullableInt56Vector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        $defaultValue = 0;
        $defaultValue = \pack('PX', $defaultValue >= 0 ? $defaultValue : -$defaultValue + 36028797018963967);
        $substringToInsert = '';
        ...
        foreach ($elements as $element) {
            if (null === $element) {
                ...
                if ($element < -36028797018963968 || $element > 36028797018963967) {
                    throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '.-36028797018963968 .' <= x <= '. 36028797018963967);
                }
                $packedInteger = \pack('PX', $element >= 0 ? $element : -$element + 36028797018963967);
    ...
    private function insertBytes(string $substringToInsert, int $firstIndex): void
    {
        ...
        if (-1 === $firstIndex || $firstIndex > $this->elementCount - 1) {
            // Insert the elements.
            $padLength = \strlen($substringToInsert) + \max(0, $firstIndex - $this->elementCount) * 7;
            $this->primarySource .= \str_pad($substringToInsert, (int) $padLength, $defaultValue, \STR_PAD_LEFT);
            $this->elementCount += $padLength / 7;
        } else {
            $originalFirstIndex = $firstIndex;
            // Calculate the positive index corresponding to the negative one.
            if ($firstIndex < 0) {
            ...
            $insertedElementCount = (int) (\strlen($substringToInsert) / 7);
            ...
            if (-$originalFirstIndex > $newElementCount) {
                ...
                $padLength = ($overflow + $insertedElementCount) * 7;
                $substringToInsert = \str_pad($substringToInsert, (int) $padLength, $defaultValue, \STR_PAD_RIGHT);
            }
            // Insert the elements.
            $this->primarySource = \substr_replace($this->primarySource, $substringToInsert, $firstIndex * 7, 0);
            $this->elementCount += (int) (\strlen($substringToInsert) / 7);
        }
    }
}
