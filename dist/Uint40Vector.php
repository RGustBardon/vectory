...
class Uint40Vector implements VectorInterface
{
    ...
    public function offsetGet($index)
    {
        ...
        return \unpack('P', $packedInteger."\0\0\0")[1];
    }
    ...
    public function offsetSet($index, $value)
    {
        ...
        $packedInteger = \pack('PXXX', $unpackedInteger);
        ...
        if ($unassignedCount < 0) {
            ...
            $this->primarySource .= \str_repeat("\0\0\0\0\0", (int) $unassignedCount).$packedInteger;
        }
    ...
    public function getIterator(): \Traversable
    {
        ...
        for ($getIteratorIndex = 0; $getIteratorIndex < $elementCount; ++$getIteratorIndex) {
            $packedInteger = \substr($clone->primarySource, $getIteratorIndex * 5, 5);
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\Uint40Vector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        foreach ($elements as $element) {
            ...
            if ($element < 0 || $element > 1099511627775) {
                throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '. 0 .' <= x <= '. 1099511627775);
            }
            $packedInteger = \pack('PXXX', $element);
    ...
    private function deleteBytes(bool $primarySource, int $firstIndex, int $howMany, int $elementCount): void
    {
        if ($howMany >= $elementCount - $firstIndex) {
            if ($primarySource) {
                $this->primarySource = \substr($this->primarySource, 0, $firstIndex * 5);
                $this->elementCount = $firstIndex;
            }
        } else {
            if ($primarySource) {
                $this->primarySource = \substr_replace($this->primarySource, '', $firstIndex * 5, $howMany * 5);
    ...
    {
        $defaultValue = 0;
        $defaultValue = \pack('PXXX', $defaultValue);
        if (-1 === $firstIndex || $firstIndex > $this->elementCount - 1) {
            ...
            $insertedElementCount = (int) (\strlen($substringToInsert) / 5);
