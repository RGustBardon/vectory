...
class Int32Vector implements VectorInterface
{
    ...
    public function offsetGet($index)
    {
        ...
        $packedInteger = \substr($this->primarySource, $index * 4, 4);

        return \unpack('l', $packedInteger)[1];
    }
    ...
    public function offsetSet($index, $value)
    {
        ...
        if ($value < -2147483648 || $value > 2147483647) {
            throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$value.', expected '.-2147483648 .' <= x <= '. 2147483647);
        }
        $unpackedInteger = $value;
        $packedInteger = \pack('l', $unpackedInteger);
        ...
        if ($unassignedCount < 0) {
            ...
            $this->primarySource .= \str_repeat("\0\0\0\0", (int) $unassignedCount).$packedInteger;
        }
    ...
    public function jsonSerialize(): array
    {
        ...
        $elementCount = $this->elementCount;
        for ($getIteratorIndex = 0; $getIteratorIndex < $elementCount; ++$getIteratorIndex) {
            $packedInteger = \substr($this->primarySource, $getIteratorIndex * 4, 4);
            $element = \unpack('l', $packedInteger)[1];
            $result[] = $element;
        }
    ...
    {
        return \serialize([self::SERIALIZATION_FORMAT_VERSION, \pack('S', 1) === \pack('v', 1), $this->elementCount, $this->primarySource]);
    }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\Int32Vector', '\\')]]);
        \restore_error_handler();
        if (false === $newValues) {
            throw new \UnexpectedValueException(self::EXCEPTION_PREFIX.\sprintf('Failed to unserialize (%s)', $errorMessage));
        }
        $expectedTypes = ['integer', 'boolean', 'integer', 'string'];
        ...
        [$version, $littleEndian, $this->elementCount, $this->primarySource] = $newValues;
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        $defaultValue = 0;
        $defaultValue = \pack('l', $defaultValue);
        $substringToInsert = '';
        foreach ($elements as $element) {
            ...
            if ($element < -2147483648 || $element > 2147483647) {
                throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '.-2147483648 .' <= x <= '. 2147483647);
            }
            $packedInteger = \pack('l', $element);
