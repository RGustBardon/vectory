...
class NullableUint56Vector implements VectorInterface
{
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\NullableUint56Vector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        $defaultValue = 0;
        $defaultValue = \pack('PX', $defaultValue);
        $substringToInsert = '';
        ...
        foreach ($elements as $element) {
            if (null === $element) {
                ...
                if ($element < 0 || $element > 72057594037927935) {
                    throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '. 0 .' <= x <= '. 72057594037927935);
                }
                $packedInteger = \pack('PX', $element);
