...
class NullableUint32Vector implements VectorInterface
{
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\NullableUint32Vector', '\\')]]);
        ...
        foreach ($elements as $element) {
            if (null === $element) {
                ...
                if ($element < 0 || $element > 4294967295) {
                    throw new \OutOfRangeException(self::EXCEPTION_PREFIX.'Value out of range: '.$element.', expected '. 0 .' <= x <= '. 4294967295);
                }
