...
class NullableChar2Vector implements VectorInterface
{
    ...
    public function offsetGet($index)
    {
        ...
        if ($isNull) {
            ...
            $value = \substr($this->primarySource, $index * 2, 2);
        }
    ...
    public function offsetSet($index, $value)
    {
        ...
        if ($unassignedCount < 0) {
            ...
            $byteIndex = $index * 2;
            do {
                $this->primarySource[$byteIndex++] = ($value ?? "\0\0")[$elementIndex++];
            } while ($elementIndex < 2);
        } elseif (0 === $unassignedCount) {
            // Case 2. Append an element right after the last one.
            $this->primarySource .= $value ?? "\0\0";
        } else {
            // Case 3. Append to a gap after the last element. Fill the gap with default values.
            $this->primarySource .= \str_repeat("\0\0", (int) $unassignedCount).($value ?? "\0\0");
        }
    ...
    public function getIterator(): \Traversable
    {
        ...
        for ($index = 0, $lastIndex = $elementCount & ~7; $index < $lastIndex;) {
            ...
            $primaryBatch = \str_split(\substr($primarySource, $index * 2, $batchSize * 2), 2);
        ...
        for (; $index < $elementCount; ++$index) {
            if ("\0" === ($nullabilitySource[$index >> 3] & $mask[$index & 7])) {
                (yield \substr($primarySource, $index * 2, 2));
            } else {
                yield;
            }
        }
    ...
    public function jsonSerialize(): array
    {
        ...
        for (; $index < $elementCount; ++$index) {
            if ("\0" === ($nullabilitySource[$index >> 3] & $mask[$index & 7])) {
                $jsonData[] = \substr($primarySource, $index * 2, 2);
            } else {
                $jsonData[] = null;
            }
        }
    ...
    public function unserialize($serialized)
    {
        ...
        $newValues = \unserialize($serialized, ['allowed_classes' => [\ltrim('\\Vectory\\NullableChar2Vector', '\\')]]);
    ...
    public function insert(iterable $elements, int $firstIndex = -1): void
    {
        ...
        foreach ($elements as $element) {
            if (null === $element) {
                ...
                if (2 !== \strlen($element)) {
                    throw new \LengthException(self::EXCEPTION_PREFIX.\sprintf('Value must be exactly %d bytes, %d given', 2, \strlen($element)));
                }
