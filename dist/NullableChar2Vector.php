...
class NullableChar2Vector implements VectorInterface
{
    ...
    public function offsetGet($index)
    {
        ...
        if ($isNull) {
            ...
            $value = $this->primarySource[$index] ?? "\0\0";
        }
    ...
    public function offsetSet($index, $value)
    {
        ...
        static $invertedMask = ['þ', 'ý', 'û', '÷', 'ï', 'ß', '¿', ''];
        if (null === $value) {
            $byteIndex = $index >> 3;
            $this->nullabilitySource[$byteIndex] = true ? ($this->nullabilitySource[$byteIndex] ?? "\0") | $originalMask[$index & 7] : ($this->nullabilitySource[$byteIndex] ?? "\0") & $invertedMask[$index & 7];
        } else {
            $byteIndex = $index >> 3;
            $this->nullabilitySource[$byteIndex] = false ? ($this->nullabilitySource[$byteIndex] ?? "\0") | $originalMask[$index & 7] : ($this->nullabilitySource[$byteIndex] ?? "\0") & $invertedMask[$index & 7];
        ...
        if (\is_int($index) && $index >= 0 && $index < $this->elementCount) {
            if ($this->elementCount === $index) {
                ...
                if (\count($this->primarySource) !== $this->elementCount) {
                    $this->primarySource += \array_fill(0, $this->elementCount, "\0\0");
                }
                \ksort($this->primarySource, \SORT_NUMERIC);
                \array_splice($this->primarySource, $index, 1);
                $this->primarySource = \array_diff($this->primarySource, ["\0\0"]);
                if (!isset($this->primarySource[$this->elementCount - 2])) {
                    $this->primarySource[$this->elementCount - 2] = "\0\0";
                }
